...
def gaze(frame, points):
    ...
    if transformation is not None:  # if estimateAffine3D secsseded
        ...
        S = Eye_ball_center_left + (pupil_world_cord - Eye_ball_center_left) * 5
        ...
        # cv2.circle(frame,eye_pupil2D[0][0],3,(255,0,0))
        print(eye_pupil2D[0][0]-left_pupil)
        ...
        # Draw gaze line from nose tip to gaze direction for the left pupil
        # nose_tip = (int(image_points[0][0]), int(image_points[0][1]))
        # gaze_line_end = (int(gaze[0]), int(gaze[1]))

        # cv2.line(frame, nose_tip, gaze_line_end, (0, 0, 255), 2)
        ...
        if 0 > (p1[0] - p2[0]) :
            print("right")
            cv2.putText(frame, 'right', (int(w-100),int(h/2)), cv2.FONT_HERSHEY_SIMPLEX ,  
                   1, (255, 0, 0) , 2, cv2.LINE_AA)
        if 0 < (p1[0] - p2[0]) :
            print("left")
            cv2.putText(frame, 'left', (int(100),int(h/2)), cv2.FONT_HERSHEY_SIMPLEX ,  
                   1, (255, 0, 0) , 2, cv2.LINE_AA)
        if 0 < (p1[1] - p2[1]) :
            print("up")
            cv2.putText(frame, 'up', (int(w/2),int(100)), cv2.FONT_HERSHEY_SIMPLEX ,  
                   1, (255, 0, 0) , 2, cv2.LINE_AA)
        if 0 > (p1[1] - p2[1]) :
            print("down")
            cv2.putText(frame, 'down', (int(w/2),int(h-100)), cv2.FONT_HERSHEY_SIMPLEX ,  
                   1, (255, 0, 0) , 2, cv2.LINE_AA)
